...
| `LolstatsRiotKeyOld` (`lolstats-riot-key`, the whole group) | **Dead gauge.** `lolstats_riot_key_age_seconds` is scraped as a constant `0` while `/readyz` reports the true age (observed `138s` and `10980s` at different moments, `/metrics` reading `0` in both). Its only writer sits behind a two-value `Age() (time.Duration, bool)` assertion that only the test fake satisfies (`internal/crawl/fakes_test.go`), never the real ingest client, so CI does not catch it. | a one-value `Age() time.Duration` on the real provider, or a writer that does not go through the assertion. The two-value form at `internal/crawl/worker.go:916` never matches `internal/riot.KeyProvider.Age()` (`internal/riot/key.go:101`), which returns one value, so the gauge keeps its `0` default - re-measured live 2026-09-18 with the same `0` while `/readyz` reported a real age. **The expiry *input* is no longer missing:** `LOLSTATS_RIOT_API_KEY_EXPIRES_AT` is parsed by `internal/config` and *is* consumed (`internal/riot/expiry.go`, wired at `cmd/lolstats-ingest/main.go:212,396,437` and `internal/crawl/crawl.go:69`, and declared in the ingest, backfill and discover-seeds manifests since `98e37be`), so a key with a recorded deadline fails before its first refused call. The gauge that would let an alert watch the age is the part still missing. Until it is fixed the observable symptom of an expiry is a sustained auth rejection, which is what `LolstatsRiotKeyRevoked` watches, and the practical control is a calendar alarm (`docs/runbooks/key-rotation.md`). |
