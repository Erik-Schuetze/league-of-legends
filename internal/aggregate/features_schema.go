...
func featureReadme(counts FeatureCounts, opts FeatureOptions) string {
	...
	write("### KDA at 5, 10 and 15 minutes versus the opponent\n\n")
	write("```sql\n")
	write("SELECT\n  role,\n  count(*) AS n,\n")
	write("  avg((kills_10 - deaths_10)::DOUBLE) AS avg_kda_diff_10,\n")
	write("  avg(win::INT) AS win_rate\n")
	write("FROM read_parquet('/path/to/dataset/lane_matchups/*.parquet')\n")
	write("WHERE kills_10 IS NOT NULL\n")
	...
	write("image pixels, so a plot needs the map's own transform.\n\n")
