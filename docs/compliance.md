...
**Status: met for the `agg/v1` artifact set; the trigger is live for anything
new.** The published shape is `internal/aggmodel/schema.go` and
`schema/agg.schema.json`, generated by `go run ./cmd/gen-types`: a tier list,
champion statistics and matchups, all derived aggregates carrying their own `n`.
That is the whole `agg/v1` reader contract, and obligations 1 and 9 re-prove on
every change that it carries no rating-like value and nothing per-player.

**A second derived dataset, `timeline-v1`, was added under this trigger.** It is
built by `lolstats-aggregate features` from the match summary and timeline
archives, and published under `<LOLSTATS_AGG_DATASET_ROOT>/timeline-v1` (default
root `/var/lib/lolstats/datasets`). It sits outside the `agg/v1` reader contract
rather than extending it, and it does not change what `agg/v1` publishes. Its
position is derived, non-identifying statistics: **no `puuid` appears in any
...
| Licence inventory | **not met** | No inventory file and no `make` target produce one. With the frontend toolchain gone there are no npm dependencies at all, which makes the exposure smaller than it was, but "small" is not a record |

**Next step (owner).** Generate a licence inventory for the Go module graph,
commit it, and add a check that fails when a new dependency appears without a
recorded licence. Until then this row stays open, and any new dependency should be
...
**Status: not applicable.** No monetisation is planned or implemented. There is no
checkout route, no subscription route and no pricing page anywhere in the project,
so the free-tier requirement and the no-monetisation posture are the same fact
today.
...
## The launch-blocking claims, and what is left of their proof

Every claim below was reproducible offline, with no network and no `npm install`,
by a check in the deleted gate. The automated form is gone; the claims are
reproducible against the data layer where they still apply.

### The free tier is genuinely free and ungated

No account, no login, no paywall, no rate-limited teaser, no email capture, and
every control the page offers changes the document without JavaScript. This is a
property of the serving layer, which does not exist, so it is carried forward as
obligation 4 rather than asserted.

There is no auth code anywhere in the project, no session cookie, and no identity
provider in the deployment. The durable half is the no-JS requirement: a future
filter bar must be a `method="get"` form with an on-origin `action`, and every
named control must sit inside a form. A control with no `name` cannot submit
anything and is a JavaScript enhancement on top of a working page.

### No MMR, ELO or rating-like value is computed, stored or displayed anywhere

This is a hard Riot prohibition, so it holds as an absence across every language
the project uses.

```
grep -rniE 'mmr|elo|skill_rating|matchmaking_rating|player_rating|hidden_rating' \
  --include='*.go' --include='*.sql' --include='*.json' --include='*.ts' .
```

The only expected hits are negations of the prohibition in this page and its
history. The rule for a future serving layer is that a rating-like identifier,
key, column or displayed value fails obligation 1. The published artifact schema
in `schema/` is the whole published shape and carries no such field, so no page
built from it could display one.

### The privacy policy describes what the implementation actually does

The policy must be written from the implementation rather than from a template,
so that each of its claims is checkable. The claims the retired policy made, and
which a replacement must be able to make truthfully, are:

| Claim | Why it was true |
| --- | --- |
| No cookies, and none set on the site's behalf | Nothing in the deployment sets one, and no `document.cookie` existed in the serving code |
| No analytics, advertising, tracking pixel or third-party embed | Every executable resource was same-origin; obligation 3 |
| No accounts, logins, forms or user submissions | No form was anything but a same-origin GET filter |
| Nothing stored on the device | No `localStorage`, `sessionStorage` or cookie use anywhere |
| The access log is the only processing | The server wrote one line per request to its own stderr, and **no log shipper, no log store and no retention configuration exists**, which is why the practical retention is days, until the container is replaced |
| The Data Dragon icon request is disclosed | The policy named `ddragon.leagueoflegends.com` and stated that it receives the visitor's IP address and user agent, that Riot Games is established in the United States, and that this is therefore a transfer outside the EEA |

The retired policy also disclosed a near miss deliberately: the operator runs a
self-hosted Umami analytics instance elsewhere on the same home cluster at a
different hostname. The site loaded no script from it and sent it nothing. That
was stated on the page anyway, because a reader who discovered the instance
themselves would reasonably wonder, and a disclosure that only covers what is
definitely fine is not a disclosure. A replacement policy should keep that
disclosure.

### No data-broker behaviour

The project publishes derived aggregates only. It does not resell or expose the
raw archive, and it publishes no per-player identifiable data.

The raw archive - verbatim Riot payloads - lives on the cluster and is never
fetched by a visitor, and it is not published as an artifact. The published
artifact schema declares no PUUID, no summoner id, no account id, no Riot id and
no profile icon id. That field is addressed by MATCH-V5, which is why the control
plane holds it and the published surface does not. The `timeline-v1` dataset adds
...
carries a player dimension. The aggregates are role, champion, rank bracket,
region, queue and patch - never a person. Committed payloads carry only synthetic
identifiers, all either the reserved `fixture-` prefix or the reserved `FIXT` Riot
ID tagline that the fixture generator writes, and `fixtures/README.md` states
that nothing in the directory is real Riot data.
...
Against a future serving layer:

```
grep -rhoE 'https?://[A-Za-z0-9.-]+' <served pages> --include='*.html' --include='*.css' \
  | tr '[:upper:]' '[:lower:]' | sort -u
```

Every origin that appears in an image, icon, `og:image`, `twitter:image`,
`<source>` or CSS `url()` position must be `ddragon.leagueoflegends.com/cdn/`, and
the remaining references must be same-origin. The retired site's last capture
carried 1038 `<img>` tags on the Data Dragon CDN and zero other absolute image
origins - a dated reading of that day's corpus, not a floor.

No build fetches Data Dragon. The static data is checked in under `projection/`
and regenerated from the public CDN by hand, and the frozen artifact tree
reserves a prefix for it (`docs/contracts.md` section 4). Nothing in the running
pipeline makes an outbound Riot request to render data, and no visitor's page
view causes a Riot request.

## Standing constraints

These are properties of the design rather than steps, and a change that breaks one
of them is a decision that needs an ADR:

- **No request-time Riot API access from a serving layer.** Anything served reads
...
- **No paid tier.** The published data is free and needs no account.
...
- **The data state is disclosed on the page, not inferred by the reader.** Every
  page must carry the patch, region, queue and bracket it was built from, plus the
  aggregate manifest's `source` (`demo`, `riot-match-v5`, or no data).
