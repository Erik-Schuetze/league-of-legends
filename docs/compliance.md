...
The published shape is `internal/aggmodel/schema.go` and
...
Nothing beyond that is published, and obligations 1 and 9 re-prove on every
change that it carries no rating-like value and nothing per-player.
...
plane holds it and the published surface does not.
