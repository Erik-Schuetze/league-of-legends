...
| `site/` | published artifact tree (`v1/manifest.json`, `v1/p/<patch>/...`, `v1/static/<version>/...`) | The demo tree a reader can point at, generated from the payloads above by the aggregation tests rather than hand-authored, so every identifier in it is one of theirs and it carries no player data for the same reason they do not. It was moved here from `web/src/fixtures/` on 2026-09-17, and it was the tree the Go tier rendered while `LOLSTATS_AGG_FIXTURES=only` was set - a variable and a Deployment that no longer exist, because the tier was retired on 2026-09-18 (`docs/decisions/ADR-011-retire-the-web-tier.md`). Nothing reads this directory now: no code path names it, no manifest mounts it and no test asserts on it. It is kept as a worked example of the artifact contract, not as an input to anything |
