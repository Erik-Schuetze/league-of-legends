...
the repository under `projection/` and duplicated at `internal/webtier/data/*.json`,
which `internal/webtier/data.go` embeds so the deployed binary is self-contained
(`data.go` says the two copies are byte-identical and that a drift test guards
them; no such test is present, so nothing enforces it - reported, not fixed here). No build-time fetch remains - `web/scripts/fetch-ddragon.mjs`
